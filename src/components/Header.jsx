...
export default function Header() {
  ...
  return (
    <header className={`w-full fixed top-0 z-50 transition-all duration-300 ${
      scrolled ? "bg-zinc-950/80 backdrop-blur-md shadow-lg" : "bg-zinc-950"
    }`}>
      ...
      <div className={`border-b border-white/5 transition-all duration-300 ${scrolled ? "py-3" : "py-5"}`}>
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          <a href="#" className="flex items-center gap-2 group">
            <div className="w-9 h-9 rounded-lg bg-gradient-to-tr from-emerald-600 to-amber-400 flex items-center justify-center font-black text-black text-lg">
              A
            </div>
          ...
          <nav className="hidden lg:flex items-center gap-8">
            ...
            {["Services", "Investment Plans", "Insights", "About Us", "Contact"].map((item) => (
              <a 
                key={item} 
                href="#" 
          ...
          <div className="hidden lg:flex items-center gap-4">
            ...
            <button className="bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-medium px-6 py-2.5 rounded-xl shadow-lg shadow-emerald-900/20 hover:shadow-emerald-600/20 hover:-translate-y-0.5 transition active:translate-y-0">
              Book Consultation
            </button>
          </div>
      ...
      <div className={`lg:hidden fixed inset-0 top-[110px] bg-zinc-950/95 backdrop-blur-lg transform transition-transform duration-300 ease-in-out z-40 ${
        ...
        <nav className="flex flex-col p-6 gap-6 text-lg border-t border-white/5">
          ...
          <button className="bg-emerald-600 text-white font-medium py-3 rounded-xl shadow-lg text-center">
            Book Consultation
          </button>
          <button className="bg-white/5 text-white font-medium py-3 rounded-xl border border-white/10 text-center">
            Log In
          </button>
        </nav>
      </div>
